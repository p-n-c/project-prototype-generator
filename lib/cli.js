...
async function generateProject({
  ...
  try {
    ...
    const testSetup = new TestSetup(projectPath, __dirname)
    ...
    console.log('Required project files:', requiredFiles)
    ...
    const packageJson = {
      ...
      type: 'module',
      scripts,
      // scripts: projectConfig.scripts.base,
      source:
        projectType === 'web-next' ? ['./app/page.jsx'] : ['./src/index.html'],
      devDependencies,
      // devDependencies: projectConfig.dependencies.base,
    }

    // Add test configurations if needed
    if (includeUnitTests || includeE2ETests) {
      await testSetup.setupProjectTests(projectConfig.id, {
        includeUnitTests,
        includeE2ETests,
      })
