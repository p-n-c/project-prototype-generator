...
import { getPrototypeTypes } from './project-definitions-api.js'
...
export class FileOperations {
  ...
  async copyPromptTemplate(projectType, template, projectPath, options) {
    try {
      ...
      prompt.thread.promptTemplate.project.title = options.projectTitle
      prompt.thread.promptTemplate.project.description =
        options.projectDescription

      const prototypes = getPrototypeTypes()
      const prototype = prototypes.find(
        (prototype) => prototype.value === options.prototype
      )

      prompt.thread.promptTemplate.project.definition = prototype.humanPrompts

      prompt.thread.promptTemplate.prototype.type = prototype.response
      prompt.thread.promptTemplate.prototype.typeDefinition =
        prototype?.choice?.description
