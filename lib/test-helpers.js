import fs from 'fs-extra'
import path from 'path'

import { getProjectConfigByType } from './project-types.js'

export class TestSetup {
  constructor(projectPath, templateRoot) {
    this.projectPath = projectPath
    this.templateRoot = templateRoot
  }

  async setupProjectTests(projectConfig, options) {
    const { includeUnitTests, includeE2ETests } = options

    if (includeUnitTests) {
      await this.setupUnitTests(projectConfig)
    }

    if (includeE2ETests) {
      await this.setupE2ETests(projectConfig)
    }
  }

  async setupUnitTests(projectConfig) {
    const config = getProjectConfigByType(projectConfig.type)
    const testConfig = path.join(
      this.templateRoot,
      'configs/jest',
      `${config.id}.json`
    )

    await fs.copy(testConfig, path.join(this.projectPath, 'jest.config.json'))
    await this.copyTestTemplates(projectConfig)
  }

  async setupE2ETests(projectConfig) {
    const cypressStructure = [
      'cypress/e2e',
      'cypress/fixtures',
      'cypress/support',
    ]

    for (const dir of cypressStructure) {
      await fs.ensureDir(path.join(this.projectPath, dir))
    }

    await this.copyE2ETemplates(projectConfig)
  }
}
