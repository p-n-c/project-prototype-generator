...
export class TestSetup {
  constructor(projectPath, templateRoot, srcFolder) {
    ...
    this.configsPath = path.join(templateRoot, 'configs')
  }
  ...
  async setupProjectTests(projectConfig, options) {
    ...
    if (includeE2ETests) {
      // let's assume for now, we reply on the cypress set up to generate an example test/spec
    }
  }
  ...
  async setupNextJsTestConfig() {
    ...
    await fs.copy(
      path.join(this.configsPath, 'next/jest.setup.js'),
      path.join(this.projectPath, 'jest.setup.js')
    )

    await fs.copy(
      path.join(this.configsPath, 'next/babel.config.json'),
